...
## 🔐 Авторизация (аккаунты + админ-доступ)

Пользовательские функции (поиск, ассистент, услуги, партнёры) — **публичные**.
Админ-панель (загрузка, каталог, очереди проверки, дашборд) — **только для админов**.

- Регистрация / вход: `/login` и `/register` (веб) или `POST /api/auth/register`,
  `POST /api/auth/login`. Новые аккаунты всегда получают роль `user` — роль нельзя
  задать из запроса.
- Дефолтный админ создаётся при первом запуске: **`admin@medarchive` / `admin12345`**
  (переопределяется `ADMIN_PASSWORD`; для боевого окружения задайте `AUTH_SECRET`).
- Повысить существующий аккаунт до админа: `python -m scripts.make_admin <email>`.
- Все эндпоинты `/api/admin/*` требуют админский bearer-токен: `401` без токена,
  `403` для не-админа. Токены — HMAC-подписанные, со сроком жизни; пароли —
  pbkdf2-sha256 (200k итераций, соль на пользователя). Без новых зависимостей.
